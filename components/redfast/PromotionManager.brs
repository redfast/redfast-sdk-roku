    ...
    for ii = 0 to paths.count() - 1
        ...
        for jj = 0 to path.triggers.count() - 1
            ...
            if useRegex <> invalid and useRegex
                ...
                checkTriggerScreenName = matchWildCharStrings(urlPath, m.currentScreenName)
            end if
...
sub onModalDismissed()
    ...
    closedPath = m.modalParam.path
    pathTypes = PathType()
    if closedPath.path_type = pathTypes.modal or closedPath.path_type = pathTypes.bottomBanner or closedPath.path_type = pathTypes.interstitial
        if closedPath.sequence <> invalid
            reason = ""
            if m.modalResult.code = m.allResults.button1
                if closedPath.actions.rf_retention_button1_text <> invalid
                    reason = closedPath.actions.rf_retention_button1_text
                end if
            else if m.modalResult.code = m.allResults.button2
                if closedPath.actions.rf_retention_button2_text <> invalid
                    reason = closedPath.actions.rf_retention_button2_text
                end if
            else if m.modalResult.code = m.allResults.button3
                reason = "declined"
            else if m.modalResult.code = m.allResults.dismissed
                reason = "dismissed"
            else if m.modalResult.code = m.allResults.timeout
                reason = "timeout"
            end if

            sequence = closedPath.sequence
            shouldContinue = false
            for ii = 0 to sequence.continue_interaction_types.count() - 1
                if sequence.continue_interaction_types[ii] = reason
                    shouldContinue = true
                    exit for
                end if
            end for
            ...
            if shouldContinue
                ...
                if m.actions <> invalid and m.actions.paths <> invalid
                    for ii = 0 to m.actions.paths.count() - 1
                        p = m.actions.paths[ii]
                        if p.sequence <> invalid and p.sequence.id = sequence.id and p.sequence.order = sequence.order + 1
                            nextPath = p
                            exit for
                        end if
                    end for
                ...
                if nextPath <> invalid
                    nextPath.delay_seconds = 0
                    prepareModal(nextPath, m.modalParam.root)
                end if
            end if
        end if
    end if
...
sub prepareModal(path as object, root as object)
    if path <> invalid
        modalParam = {}
...
sub onDisplayModal()
    ...
    m.top.result = preparePromptResult({
        code: m.allResults.impression,
        meta: m.modalParam.path.actions.rf_metadata,
        deeplink: m.modalParam.path.actions.rf_settings_deeplink
    }, m.modalParam.path)
    ...
    if m.modalParam.path.path_type = pathTypes.modal or m.modalParam.path.path_type = pathTypes.interstitial or m.modalParam.path.path_type = pathTypes.bottomBanner
        if m.modalParam.path.actions.rf_retention_survey_selected = "true"
            m.modal = createObject("roSGNode", "PromotionSurvey")
        else
...
function matchWildCharStrings(trigger as string, name as string) as boolean
    if trigger = name or trigger = "*" or name = "*"
        return true
    end if

    if trigger = invalid or name = invalid
        return false
    end if

    specialChars = ["?", "+", ".", "(", ")", "[", "]", "{", "}", "\", "^", "$", "|"]
    for each char in specialChars
        trigger = trigger.replace(char, "\" + char)
    end for
    trigger = trigger.replace("*", ".*")
    pattern = CreateObject("roRegex", trigger, "")
    match = pattern.IsMatch(name)
    return match
end function
...
function getPrompts(params as object) as object
    ...
    zid = params.zoneId
    zoneIdName = ""
    if zid <> invalid then zoneIdName = zid
    pathTypes = PathType()
    prompts = []
...
function getTriggerablePrompts(params as object) as object
    ...
    prompts = getPrompts({ pzType: pzType, zoneId: zid })
    filteredPrompts = []
    for ii = 0 to prompts.count() - 1
        ...
        if prompt.pathItem.triggers <> invalid
            for jj = 0 to prompt.pathItem.triggers.count() - 1
                ...
                matchedScreenName = matchWildCharStrings(trigger.url_path, screenName)
                matchedClickId = matchWildCharStrings(trigger.click_id, clickId)
